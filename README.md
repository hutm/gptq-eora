...
Reference implementation of {2,3,4}-bit CUDA kernels for "EoRA: Training-free Compensation for Compressed LLM with Eigenspace Low-Rank Approximation" (https://arxiv.org/abs/2410.21271) paper. 
...
by forking `gemm_half_q_half_gptq_{2,3,4}bit_kernel` into `gemm_half_q_half_gptq_{2,3,4}bit_kernel_eora`, which accepts additional input: `Ax` and `B` matrices along with LORA rank.
...
Speedup for 4-bit eora kernel is ranging between ~2.3x and 1.2x is observed for batch sizes ranging from 1 to 16 on a single RTX 3090 GPU. 
